...
class MaxAbsScaler(DataScaler):
    """
    MaxAbsScaler实现，继承自DataScaler类
...
def process_data(input_file, output_file):
    ...
    try:
        ...
        if len(complete_data) == 0:
            logger.error("没有找到完整的数据行，无法进行归一化")
            return
        ...
        df_imp = pd.DataFrame(df_imp, columns=df.columns)  # 使用原始列名
...
if __name__ == "__main__":
    ...
    input_file = os.path.join(current_dir, "juhe.csv")
